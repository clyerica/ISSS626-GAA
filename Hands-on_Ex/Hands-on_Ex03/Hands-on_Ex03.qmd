...
-st_transform(crs=32748) transforms the projections to the correct projected coordinate system (WGS 84) for areas in Indonesia south of the equator.  
:::
```{r}
plot(kbb_sf)
...
- st_as_sf() converts the listing dataframe into a simple data frame
  ...
  - crs: provides the coordinates system in epsg format
-st_transform(crs=32748) transforms the projections to the correct projected coordinate system (WGS 84) for areas in Indonesia south of the equator.  
:::
