...
In this hands-on exercise, we will compute spatial weights using R. By the end to this hands-on exercise, we will demonstrate the following capabilities:
...
In this section, we will use poly2nb() of spdep package to compute contiguity weight matrices for the study area. This function builds a neighbours list based on regions with contiguous boundaries. A “queen” argument that takes TRUE or FALSE as options can be passed into the function. If queen = FALSE is not specified, this function will return a list of first order neighbours using the Queen criteria.
...
> Ans: Average number of links refers to the average number of neighbours each point has within the minimum and maximum distance range specified in the function.
...
> Answer: Row-standardised weights allow us to view a comparable value across all locations, regardless of the number of neighbours for each location.
...
> Answer: Using the sum of neighbouring values instead of row standardised weights allow us to view the total magnitude of the neighbouring effect, and acoids down-weighting the influence of neighbours in areas with many neighbours.
