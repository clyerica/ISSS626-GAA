...
::: callout-warning
Except `MP14_SUBZONE_NO_SEA_PL` data set, the other three data set are specially prepared by Prof. Kam for teaching and research purpose. Students taking IS415 Geospatial Analytics and Applications are allowed to use them for hands-on exercise purpose.
:::
...
![The default field name is very messy, we will rename it to `accHansen` by using the code chunk below.](images/clipboard-2846118326.png)
...
![](images/clipboard-1168381316.png)

The steps above can be condensed into the single code chunk as below
...
In this section, weare going to repeat most of the steps in the previous section to perform the analysis. However, some of the codes will be combined into one code chunk.\
...
Since there are outliers which skews our visualisation of the data, we will plot the distribution of accSAM.
...
Most values of accSAM lie below 50. Hence, we will use this value to filter the existing dataset and compare the SAM accessibility values by URA planning region

```{r}
#| fig-width: 8
hexagon_filtered <- hexagon_SAM %>%
  filter(accSAM < 50) 
ggplot(data = hexagon_filtered, 
       aes(y = accSAM, 
           x= REGION_N)) +
  geom_boxplot() +
  geom_point(stat="summary", 
...
Now, it is easier to see that north and west region appears to have the lowest mean SAM accessibility value. The central region has the highest mean SAM accessibility value, but also has the largest spread of values. The distributions across all regions display a right skew, indicating that there are specific areas in the region with much higher accessibility than others, but majority of areas have lower accessibility values.
