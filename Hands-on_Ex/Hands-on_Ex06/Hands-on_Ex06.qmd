...
-   convert simple feature data.frame into R’s SpatialPolygonDataFrame object by using the **sf** package;

-   perform cluster analysis by using *hclust()* of Base R;

-   toperform spatially constrained cluster analysis using *skater()* of Base R; and
...
> Observations: Distribution of radio ownership is right skewed, whereas the radio penetration rate distribution more closely approximates a normal distribution.
...
TT_HOUSEHOLDS.map <- tm_shape(shan_sf) + 
  tm_polygons("TT_HOUSEHOLDS",
          fill.scale = tm_scale_intervals(n=5, style = "jenks"), 
          fill.legend = tm_legend(title = "Total households")) + 
  tm_borders(fill_alpha = 0.5) +
  tm_layout(legend.position = c("right", "top"))

RADIO.map <- tm_shape(shan_sf) + 
  tm_polygons(fill = "RADIO",
          fill.scale = tm_scale_intervals(n=5, style = "jenks"), 
          fill.legend = tm_legend(title = "Number Radio")) + 
  tm_borders(fill_alpha = 0.5) +
  tm_layout(legend.position = c("right", "top"))
...
Notice that the chloropleth maps above clearly show that townships with relatively larger number of households are also showing relatively higher number of radio ownership.

Now let us plot the chloropleth maps showing the distribution of total number of households and Radio penetration rate by using the code chunk below.
...
tm_shape(shan_sf) +
  ...
  tm_layout(outer.margins=0, asp=0, legend.position = c("right", "top"))
```

> While towns with more households exhibit relatively higher numbers of radio ownership, the choropleth plot of radio ownership highlights regions where radio penetration is high despite relatively lower total number of households in the town. This may suggest that radio penetration is affected by other factors aside from total household population.
...
In general, multiple variables will be used in cluster analysis. It is not unusual their values range are different. In order to avoid the cluster analysis result is biasedto clustering variables with large values, it is useful to standardise the input variables before performing cluster analysis.
...
The code chunk below plota the scaled *Radio_PR* field.
...
Next, *qtm()* of **tmap** package is used to plot the chloropleth map showing the cluster formed.

```{r}
qtm(shan_sf_cluster, "CLUSTER")
```

The chloropleth map above reveals the clusters are very fragmented. The is one of the major limitation when non-spatial clustering algorithm such as hierarchical cluster analysis method is used.
