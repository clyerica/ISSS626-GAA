...
In this hands-on exercise, we will compute Local Measures of Spatial Autocorrelation (LMSA) by using [sfdep](https://sfdep.josiahparry.com/) package. By the end to this hands-on exercise, we will:
...
Before we get started, we need to ensure that sf, sfdep, tmap and tidyverse packages of R are currently installed in R.
...
::: callout-note
Using equal interval classification, there is a single tight cluster around the northeast. There is another smaller cluster in the southwest and a possible outlier in the central area.

Using quantile interval classification, it appears that higher GDPPC values are clustered toward the east region. There are only a few regions in the west with higher GDPCC unlike their surrounding regions.
:::
...
### 4.3.0 Mapping the local Moran’s I
...
::: callout-note
Note that a new class called *Insignificant* has been added for records whereby p_ii \> 0.05
:::
...
> Based on the above map, the spatial clustering observed in the southeast region is statistically significant.
...
::: callout-note
Noted that for Gi\* `include_self()` must be used to endure that $w_{ii}(d)\ne0$
:::
...
In this section, we will compute adaptive distance weight matrix by using `st_dist_band()` and `st_weights()` of sfdep package.
