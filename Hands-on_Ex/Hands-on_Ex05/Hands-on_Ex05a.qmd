...
In this hands-on exercise, we will compute Global Measures of Spatial Autocorrelation (GMSA) by using **spdep** package. By the end pf this hands-on exercise, we will:
...
Next, we need to assign weights to each neighboring polygon. In our case, each neighboring polygon will be assigned equal weight (style=“W”). This is accomplished by assigning the fraction 1/(#ofneighbors) to each neighboring county then summing the weighted income values. While this is the most intuitive way to summaries the neighbors’ values it has one drawback in that polygons along the edges of the study area will base their lagged values on [fewer polygons]{.underline} thus potentially [over- or under-estimating the true nature of the spatial autocorrelation]{.underline} in the data. For this example, we’ll stick with the style=“W” option for simplicity’s sake but note that other more robust options are available, notably style=“B”.
...
-   **H₀:** There is no spatial autocorrelation in the data; GDP per capita is randomly distributed or dispersed 

-   **H₁:** There is significant spatial autocorrelation in the data; GDP per capita are not randomly distributed or dispersed but clustered.

> The Moran I statistic value of around 0.301 is greater than the expected value of -0.011. Since the p-value is \<0.01, we can conclude at 99% confidence level that the data displays spatial clustering of GDP per capita, i.e., regions with similar GDP per capita are clustered together..  
...
> Since the p-value is \<0.01, we can conclude at 99% confidence level that the data displays spatial clustering of GDP per capita, i.e., regions with similar GDP per capita are clustered together.
...
Using ggplot2 to plot the histogram instead:

```{r}
ggplot(data = data.frame(res = bperm$res), aes(x = res)) +
  geom_histogram(bins = 20, fill = "lightblue", color = "black", alpha = 0.7) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Histogram of Simulated Moran's I", x = "Simulated Moran's I", y = "Frequency") +
  theme_minimal()
```
...
> The C statistic is 0.691. Since it is \<1, this indicates spatial autocorrelation. As the p-value is \<0.01, we can conclude with 99% confidence that there is spatial autocorrelation in GDP per capita, meaning areas with similar GDP per capita are clustered together.
...
> 1.  **Positive Spatial Autocorrelation at Short Distances**:
>
>     -   **Lags 1 to 3**: The Moran's I values are positive and significant (`***` for lags 1 and 2, `*` for lag 3), indicating strong positive spatial autocorrelation at shorter distances. This means similar GDPPC values are clustered together in nearby regions.
>
>     -   The standard deviates for these lags are high, reinforcing the strong evidence of clustering.
>
> 2.  **Transition to Negative Autocorrelation**:
>
>     -   **Lag 4**: The Moran's I value becomes smaller and is not statistically significant, indicating a reduction in spatial autocorrelation.
>
>     -   **Lags 5 and 6**: These show negative Moran's I values, with significant p-values (`***` for lag 5, `**` for lag 6), suggesting negative spatial autocorrelation at greater distances. This implies dissimilar GDPPC values are more likely to occur as the distance increases.
